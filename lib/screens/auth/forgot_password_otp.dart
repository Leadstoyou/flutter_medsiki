...
import 'package:untitled/screens/auth/forgot_password_set_password_screen.dart';
...
class ForgotPasswordOtpScreen extends StatefulWidget {
  final String email;
  const ForgotPasswordOtpScreen({required this.email, Key? key}) : super(key: key);

  @override
  _OtpVerificationScreenState createState() => _OtpVerificationScreenState();
}

class _OtpVerificationScreenState extends State<ForgotPasswordOtpScreen> {
  final TextEditingController _otpController = TextEditingController();

  void verifyOtp() {
    String enteredOtp = _otpController.text.trim();
    String? savedOtp = otpForgotStorage[widget.email];

    if (enteredOtp == savedOtp) {
      showToast(message: '✅ Xác thực thành công!');
      navigate(context, ForgotPasswordSetPasswordScreen(widget.email));
    } else {
